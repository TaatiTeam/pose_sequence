...
def get_seq_rotation(
    ...
    Results will be returned in the space (rotate_dim, level_dim)
    [e.g. (z, y) by default],
    and the rotation will only work in this space.

    Args:
        pose_seq (pose2gait.PoseSequence): PoseSequence to be rotated
        start (int, optional): Frame index of pose_seq to use as start point.
        ...
        joint_name (str, optional): Name of the joint to level out.
            Defaults to 'MidHip'.
        level_dim (int, optional): Dimension to equalize over time.
            Defaults to 1, or the y dimension.
        rotate_dim (int, optional): Dimension to rotate with regard to.
            Defaults to 2, or the z dimension.
...
def angle_between_two_vectors(v1, v2):
    ...
    Args:
        v1 (list or tuple of float): Iterable with two float elements
            representing a vector
        v2 (list or tuple of float): Iterable with two float elements
            representing a vector
...
def rotate_point_around_origin(point, origin, angle, clockwise=False):
    ...
    Args:
        point (list of float): list of two float elements representing a point
            in space
        origin (list of float): list with two float elements around which to
            rotate point
        angle (float): Angle in radians by which to rotate point around origin

    Returns:
        list of float: a list of two float elements representing the rotated
            point
    """
...
def rotate_pose_seq_around_origin(
        pose_seq,
        origin,
        angle,
        dims=[2, 1],
    ...
    Args:
        ...
        origin (list of float): Two element list reprenting the point around
            which to rotate. This point is in the space defined by given dims.
        angle (float): Angle by which to rotate, in the space defined by dims.
        dims (list, optional): Dimensions in which the rotation is taking
            place. Defaults to [2, 1], which is [z, y].

    Returns:
        pose2gait.PoseSequence: The same pose sequence with every joint
            location rotated by angle around origin in the space defined
            by dims.
    """
    dim0, dim1 = dims
    ...
    for frame in joint_locs:
        ...
        for joint_loc in frame:
            ...
            rotated = rotate_point_around_origin(
                    to_rotate, origin, angle, clockwise=clockwise)
